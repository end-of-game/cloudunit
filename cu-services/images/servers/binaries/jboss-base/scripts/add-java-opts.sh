...
echo "ICIIII! JBOSS_HOME=$JBOSS_HOME dans add-java-opts"
...
sed -i 's/^JAVA_OPTS=.*$/JAVA_OPTS="-Dfile.encoding=UTF-8 -Xms'$MEMORY_VALUE'm -Xmx'$MEMORY_VALUE'm -XX:MaxPermSize=256m '$JAVA_OPTS'"/g' $JBOSS_HOME/bin/standalone.conf
