...
public class CheckUtils {
	...
	public boolean checkImageNoExist(String imageName) throws ManagerResponseException {
		...
		if (!imageNames.contains(imageName)) {
			log.log(Level.SEVERE, "the service you want to install doesn't exist/activated or spell check");
	...
	public boolean checkImageNoEnabled(String imageName) throws ManagerResponseException {
		...
		List<Image> images = null;
		try {
			images = JsonConverter.getImages(restUtils
					.sendGetCommand(authentificationUtils.finalHost + urlLoader.adminActions + urlLoader.imageEnabled,
		...
		if (!imageNames.contains(imageName)) {
			log.log(Level.SEVERE, "The service you want to use is not activated for this version");
			statusCommand.setExitStatut(1);
			return true;
		} else
			return false;
