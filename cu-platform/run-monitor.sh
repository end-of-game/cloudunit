...
if [ -e "$LOCK_CM" ]; then
	...
	echo ${HOME}/jdk1.8.0_91/bin/java -Xms128m -Xmx128m -jar ${HOME}/cloudunit/monitoring_scripts/cloudunitmonitor.jar $(docker inspect --format {{.NetworkSettings.IPAddress}} cuplatform_mysql_1) ${MYSQL_ROOT_PASSWORD} $(docker inspect --format {{.NetworkSettings.IPAddress}} cuplatform_redis_1) https
	${HOME}/jdk1.8.0_91/bin/java -Xms128m -Xmx128m -jar ${HOME}/cloudunit/monitoring_scripts/cloudunitmonitor.jar $(docker inspect --format {{.NetworkSettings.IPAddress}} cuplatform_mysql_1) ${MYSQL_ROOT_PASSWORD} $(docker inspect --format {{.NetworkSettings.IPAddress}} cuplatform_redis_1) https > ${HOME}/cloudunit/monitoring_scripts/cloudunitmonitor.log
