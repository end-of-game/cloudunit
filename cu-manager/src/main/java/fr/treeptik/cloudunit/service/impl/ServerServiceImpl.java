...
public class ServerServiceImpl implements ServerService {
    ...
    public Server update(Server server, String jvmMemory, String jvmOptions,
                         String jvmRelease, String location) throws ServiceException {

        Map<String, String> configShell = new HashMap<>();
        configShell.put("port", server.getSshPort());
        configShell.put("dockerManagerAddress", server.getApplication()
                .getManagerHost());
        ...
        try {
            ...
            if ("cli".equalsIgnoreCase(location)) {
                if (jvmMemory == null || jvmMemory.equalsIgnoreCase("null")) {
                    jvmMemory = server.getJvmMemory() + "";
                }

                if (jvmOptions == null || jvmOptions.equalsIgnoreCase("null")) {
                    jvmOptions = server.getJvmOptions();
                } else {
                    jvmOptions += (" " + server.getJvmOptions());
                }

                if (jvmRelease == null || jvmRelease.equalsIgnoreCase("null")) {
                    jvmRelease = server.getJvmRelease();
                }
            }
            ...
            // Changement configuration VERSION JVM
            changeJavaVersion(server.getApplication().getName(), jvmRelease);

        } catch (Exception e) {
            throw new ServiceException("Error addJVMOpts : " + e.getMessage(),
                    e);
        }
    ...
    @Override
    /**
     * Changement de la version de JAVA
     */
    public void changeJavaVersion(String applicationName, String javaVersion)
            throws CheckException, ServiceException {

        logger.info("Starting changing to java version " + javaVersion
                + ", the application " + applicationName);
        ...
        for (Server server : listServers) {
            ...
            try {
                ...
                // HACK POUR LA VERSION DE JAVA :
                // TODO : faire un truc plus propre pour gérer version majeure
                // et mineur

                javaVersion = javaVersion.equalsIgnoreCase("7") ? "jdk1.7.0_55"
                        : "jdk1.8.0_25";
                ...
                saveInDB(server);
                logger.error("java version = " + javaVersion + " - "
                        + application.toString() + " - " + server.toString(), e);
                throw new ServiceException(e.getLocalizedMessage(), e);
            }
        }
