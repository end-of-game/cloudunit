...
public class EmailUtils {
	 ...
	 * @throws MessagingException
	 */
	private Map<String, Object> defineEmailType(Map<String, Object> mapConfigEmail) throws MessagingException {
		...
		if (emailType.equals("moduleInformations")) {
			...
			mapConfigEmail = this.constructModuleInformationsEmail(mapConfigEmail);
	...
	private Map<String, Object> constructModuleInformationsEmail(Map<String, Object> mapConfigEmail)
			throws MessagingException {

		String subjectModuleInformationsEmail = messageSource.getMessage("mail.subject.module.information", null,
				Locale.ENGLISH);
		...
		MimeMessage message = (MimeMessage) mapConfigEmail.get("message");
		message.setSubject(subjectModuleInformationsEmail + ": " + module.getApplication().getName());
		mapConfigEmail.put("message", message);

		logger.info("define Email of module information ");
		logger.debug("constructModuleInformationsBody parameters : User " + user.toString() + " Module : " + module);
		...
		try {
			mapVariables.put("module_seq", module.getInstanceNumber().toString());
			...
			if (module.getName().contains("mysql")) {
				...
				mapVariables.put("internalDNSName", module.getInternalDNSName());
				template = configuration.getTemplate("emailModuleInformations-mysql.ftl");
				...
				// mapVariables.put("pgPort",
				// module.getListPorts().get("pgPort"));
				mapVariables.put("pgUser", moduleInfos.get("username"));
				mapVariables.put("pgPassword", moduleInfos.get("password"));

				mapVariables.put("internalDNSName", module.getInternalDNSName());

				template = configuration.getTemplate("emailModuleInformations-postgres.ftl");
				...
				mapVariables.put("internalDNSName", module.getInternalDNSName());
				template = configuration.getTemplate("emailModuleInformations-mongo.ftl");
				...
				template = configuration.getTemplate("emailModuleInformations-oracle-xe.ftl");
			} else if (module.getName().contains("redis")) {
				mapVariables.put("redisPassword", module.getModuleInfos().get("password"));
				mapVariables.put("internalDNSName", module.getInternalDNSName());
				template = configuration.getTemplate("emailModuleInformations-redis.ftl");
			}
		...
		logger.info("Variables inject in Email body successfully to " + user.getEmail());

		return this.writeBody(mapConfigEmail);

	}

	private Map<String, Object> constructActivationEmail(Map<String, Object> mapConfigEmail) throws MessagingException {
