...
public class ShellUtils {
	...
	public int executeShell(String command, Map<String, String> configShell,
		...
		if (nbCallRecursive > 0) {
			logger.warn("Appel récursif : " + nbCallRecursive
					+ " pour la commande : " + command);
		}

		// On évite un trop grand nombre d'appel récursifCoquille sur jboss
		if (nbCallRecursive >= 10) {
			StringBuilder msgError = new StringBuilder();
			msgError.append("command=").append(command);
			throw new RuntimeException("Impossible de joindre le container."
					+ msgError.toString());
		}
		...
		try {
			...
			logger.warn("Appel récursif SSH car impossible de joindre le container. Tempo de 10s.");
	...
	private Session getSession(String userName, String password,
			String dockerManagerAddress, String port) throws JSchException {
