    ...
    implements Serializable
{
    ...
    @AfterReturning( pointcut = "execution(* fr.treeptik.cloudunit.service.SnapshotService.create(..))"
        ...
        + " || execution(* fr.treeptik.cloudunit.service.SnapshotService.cloneFromASnapshot(..))", returning = "result" )
    ...
    @AfterThrowing( pointcut = "execution(!java.util.List fr.treeptik.cloudunit.service.SnapshotService.create(..))"
        ...
        + " || execution(!java.util.List fr.treeptik.cloudunit.service.SnapshotService.cloneFromASnapshot(..))", throwing = "e" )
    public void afterThrowingSnapshot( StaticPart staticPart, Exception e )
        throws ServiceException
    {
        ...
        {
            case createType:
                message = MessageUtils.writeAfterThrowingSnapshotMessage( e, user, createType );
                break;
            case deleteType:
                message = MessageUtils.writeAfterThrowingSnapshotMessage( e, user, deleteType );
                break;
            case cloneFromASnapshot:
                message = MessageUtils.writeAfterThrowingSnapshotMessage( e, user, cloneFromASnapshot );
                break;
        }
        if ( message != null )
        {
